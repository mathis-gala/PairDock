...
function buildPushCommand(sessionId: string): Extract<AgentCommandEnvelope, { type: 'git.pushBranch' }> {
  return {
    ...
    payload: {
      sessionId,
      commitMessage: 'feat: generated preview workflow',
    },
  };
}
...
function emitCommandWithAcknowledgement(socket: Socket, command: AgentCommandEnvelope) {
  return new Promise<{ accepted: boolean; error?: string }>((resolve) => {
    socket.emit(agentProtocolMessageEventName, command, resolve);
  });
}
...
test('BT-016: AgentClient emits preview progress and session.ready after session.prepare succeeds', async () => {
  ...
  const previewTunnel = new FailOnceClosePreviewTunnelPort();
  const sessionRunner = new SessionRunner(
    ...
    },
    {
      ...
      previewTunnelPort: previewTunnel,
    },
  );
  ...
  try {
    ...
    const closeCommand = buildCloseCommand(sessionId, 'keep-branch');
    const closeErrorPromise = waitForAgentEvent(socket, 'error');
    const failedAcknowledgement = await emitCommandWithAcknowledgement(socket, closeCommand);
    const closeError = await closeErrorPromise;

    assert.deepEqual(failedAcknowledgement, { accepted: false, error: 'tunnel close failed' });
    assert.equal(closeError.type, 'error');
    assert.equal(closeError.payload.code, 'session.close.failed');
    assert.notEqual(sessionRunner.findWorkspace(sessionId), null);

    const closedEventPromise = waitForAgentEvent(socket, 'session.closed');
    const successfulAcknowledgementPromise = emitCommandWithAcknowledgement(socket, closeCommand);
    const closedEvent = await closedEventPromise;
    const successfulAcknowledgement = await successfulAcknowledgementPromise;

    assert.deepEqual(successfulAcknowledgement, { accepted: true });
...
test('BT-031: AgentClient pushes the prepared session branch before review request creation', async () => {
  ...
  try {
    ...
    const worktreePath = join(managedRoot, sessionId);
    await writeFile(join(worktreePath, 'generated.txt'), 'generated by PairDock');
    ...
    assert.equal(
      await execGit(remotePath, ['log', '-1', '--format=%s', branchName]),
      'feat: generated preview workflow',
    );
    assert.equal(await execGit(remotePath, ['show', `${branchName}:generated.txt`]), 'generated by PairDock');
  } finally {
    await client.stop();
    await new Promise<void>((resolve) => {
      io.close(() => resolve());
...
class FailOnceClosePreviewTunnelPort extends ReadyPreviewTunnelPort {
