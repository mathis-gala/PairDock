...
test('AgentClient resumes the same harness session with validation failures and publishes only the final result', {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    assert.match(harnessPort.inputs[1]?.prompt ?? '', /PairDock validation failed/i);
    assert.doesNotMatch(harnessPort.inputs[1]?.prompt ?? '', /Do not install dependencies/i);
...
test('AgentClient bounds automatic validation repairs to two attempts', { concurrency: false }, async () => {
...
test('AgentClient does not attribute files generated by final checks to the repair turn', {
  concurrency: false,
}, async () => {
...
test('AgentClient publishes the final worktree diff after checks finish', { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.match(diffEvent.payload.diff, /Generated by validation/);
...
class WorktreeMutatingChecksRunner extends SequencedChecksRunner {
  ...
  override async run(): Promise<ChecksResult> {
    ...
    await writeFile(join(this.worktreePath, 'generated-by-check.txt'), 'Generated by validation\n', 'utf8');
    return super.run();
  }
}
